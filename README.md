...
- Linux系统（支持Ubuntu、Debian、CentOS、Alpine等主流发行版）
- Root权限
- Bash 4.0+

**特别说明**：
- Alpine Linux 完全兼容，所有功能正常工作
- 脚本会自动检测并适配不同的 Linux 发行版
- Alpine Linux 使用 BusyBox 工具集，脚本已针对性优化

### Alpine Linux 注意事项

Alpine Linux 默认不安装 `sudo`，请使用以下方式运行脚本：

```bash
# 使用 doas（Alpine 推荐方式）
doas ./setup-swap.sh -i
...
### 查看swap详细信息
```bash
# 适用于所有 Linux 系统（包括 Alpine Linux）
cat /proc/swaps

# 或使用 swapon（某些系统支持更多选项）
swapon -s
```
...
### 创建方法
- 优先使用 `fallocate`（速度更快）
- 不可用时回退到 `dd`（兼容性更好，更可靠）
- **注意**：在 ext4、XFS、btrfs 等文件系统上，`dd` 方式更可靠，因为 `fallocate` 可能创建带"洞"的文件

### 文件系统支持
- ext4, ext3, xfs: 完全支持（推荐使用 `dd` 创建 swap）
- btrfs: 需要使用 `dd` 命令
- zfs: 不推荐使用文件作为 swap
- Alpine Linux: 完全支持，与其他发行版使用相同的方法
...
### Q: 脚本支持 Alpine Linux 吗？
A: 完全支持！脚本已经过优化，可以自动检测并适配 Alpine Linux 系统。所有功能（包括 swap 创建、sysctl 配置等）在 Alpine 上都能正常工作。

**Alpine Linux 特别提示**：
- Alpine 使用 BusyBox 工具集，脚本已做专门适配
- 默认不包含 `sudo`，请使用 `doas` 或以 root 身份直接运行
- 重启后 swap 会自动挂载并生效
- 可以使用 `cat /proc/swaps` 和 `free -h` 查看 swap 状态
...
### v1.2.0 (2026-03-26)
- ✨ 新增 Alpine Linux 系统完整支持
- 🐛 修复 Alpine Linux (BusyBox) 下 `swapon --show` 命令不兼容问题
- 🔧 改用通用的 `/proc/swaps` 方法获取 swap 信息，兼容所有 Linux 发行版
- 📝 更新文档，添加 Alpine Linux 相关说明
- 🎨 改进操作系统检测逻辑，支持多种发行版识别方式
